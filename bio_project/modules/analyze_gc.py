...
def analyze(input_file, output_file):
    """
    Задание 2: Считать все записи, вычислить GC-состав и вывести 
    последовательности в порядке возрастания GC-состава.
    """
        ...
        # gc_fraction возвращает значение от 0 до 1
         try:
            ...
            gc_list.append((gc_val, info_line))
                ...
                # Если последовательность не определена (как в вашей ошибке)
                print(f"Пропуск {record.id}: Содержимое последовательности не определено (UndefinedSequenceError).")
                continue
